...
export async function getRightOfOwnerTemplateContent(id: string, organizationId: string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE id = ? AND scopeName = 'rightOfOwner' AND organizationId = ?";
    return await executeQuery(sqlQuery, [id,organizationId]);
}

export async function getRightOfOwnerTemplateContents(organizationId: string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE scopeName = 'rightOfOwner' AND organizationId = ?";
    return await executeQuery(sqlQuery,[organizationId]);
}



export async function insertRightOfOwnerTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const { status, results } = validateDataArray([templateContent], templateContentsInsertSchema, messagesEn);
    const newTemplateContent = { ...templateContent, scopeName: 'rightOfOwner' };
    if (status) {
...
export async function updateRightOfOwnerTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const { status, results } = validateDataArray([templateContent], templateContentsUpdateAndDeleteSchema, messagesEn);
    if (status) {
        const columKey = { id: templateContent.id, organizationId: templateContent.organizationId, scopeName: 'rightOfOwner' }; // Use rightOfOwner as the columKey
...
export async function deleteRightOfOwnerTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const columKey = {  id: templateContent.id,organizationId: templateContent.organizationId, scopeName: 'rightOfOwner' }; // Use userId as the columKey
