...
export async function getRightsTemplateContent(id: string, organizationId: string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE id = ? AND scopeName = 'rights' AND organizationId = ?";
    return await executeQuery(sqlQuery, [id,organizationId]);
}

export async function getRightsTemplateContents(organizationId:string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE scopeName = 'rights' AND organizationId = ?";
    return await executeQuery(sqlQuery,[organizationId]);
}



export async function insertRightsTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const { status, results } = validateDataArray([templateContent], templateContentsInsertSchema, messagesEn);
    const newTemplateContent = { ...templateContent, scopeName: 'rights' };
    if (status) {
...
export async function updateRightsTemplateContent( templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const { status, results } = validateDataArray([templateContent], templateContentsUpdateAndDeleteSchema, messagesEn);
    if (status) {
        const columKey = { id: templateContent.id,organizationId: templateContent.organizationId, scopeName: 'rights'}; // Use userId as the columKey
...
export async function deleteRightsTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const columKey = { id: templateContent.id,organizationId: templateContent.organizationId, scopeName: 'rights'}; // Use userId as the columKey
