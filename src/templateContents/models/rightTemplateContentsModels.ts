...
export async function getRightTemplateContent(id: string, organizationId: string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE id = ? AND scopeName = 'right' AND organizationId = ?";
    return await executeQuery(sqlQuery, [id,organizationId]);
}

export async function getRightTemplateContents(organizationId: string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE scopeName = 'right' AND organizationId = ?";
    return await executeQuery(sqlQuery,[organizationId]);
}



export async function insertRightTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const { status, results } = validateDataArray([templateContent], templateContentsInsertSchema, messagesEn);
    const newTemplateContent = { ...templateContent, scopeName: 'right' };
    if (status) {
...
export async function updateRightTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const { status, results } = validateDataArray([templateContent], templateContentsUpdateAndDeleteSchema, messagesEn);
    if (status) {
        const columKey = { id: templateContent.id, organizationId: templateContent.organizationId, scopeName: 'right' }; // Use right as the columKey
...
export async function deleteRightTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const columKey = {  id: templateContent.id,organizationId: templateContent.organizationId, scopeName: 'right' }; // Use userId as the columKey
