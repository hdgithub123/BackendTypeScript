...
export async function getTemplateContent(id: string, organizationId: string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE id = ? AND scopeName = 'organization' AND organizationId = ?";
    return await executeQuery(sqlQuery, [id,organizationId]);
}

export async function getTemplateContents(organizationId: string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE scopeName = 'organization' AND organizationId = ?";
...
export async function insertTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    ...
    const newTemplateContent = { ...templateContent, scopeName: 'organization' };
    if (status) {
...
export async function updateTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    ...
    if (status) {
        const columKey = { id: templateContent.id,organizationId: templateContent.organizationId, scopeName: 'organization'}; // Use userId as the columKey
