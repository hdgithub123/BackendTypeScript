...
export async function insertUserRole(userRole: userRole): Promise<{ data: Object | null, status: boolean, errorCode: string | null }> {
    return await insertObject("roles_rights", userRole);
}


export async function updateUserRole( userRole: userRole, userRoleId: userIdRoleId): Promise<{ data: Object | null, status: boolean, errorCode: string | null }> {
    return await updateObject("roles_rights", userRole, userRoleId);
}


export async function deleteUserRole( userRoleId: { [key: string]: any }): Promise<{ data: Object | null, status: boolean, errorCode: string | null }> {
    return await deleteObject("roles_rights", { userId: userRoleId.userId, roleId:userRoleId.roleId  });
}


export async function insertUserRoles(userRoles:Array<userRole>): Promise<{ data: Object | null, status: boolean, errorCode: string | null }> {
        return await insertObjects("roles_rights", userRoles);
}


export async function updateUserRoles(userRoles:Array<userRole>): Promise<{ data: Object | null, status: boolean, errorCode: string | null }> {
        return await updateObjects("roles_rights", userRoles, ["userId", "roleId"]);
}

export async function deleteUserRoles(userRoles: Array<{ [key: string]: any }>): Promise<{ data: Object | null, status: boolean, errorCode: string | null }> {
