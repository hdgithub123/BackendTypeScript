...
export async function getBranchesTemplateContent(id: string, organizationId: string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE id = ? AND scopeName = 'branches' AND organizationId = ?";
    return await executeQuery(sqlQuery, [id,organizationId]);
}

export async function getBranchesTemplateContents(organizationId:string) {
    const sqlQuery = "SELECT * FROM template_contents WHERE scopeName = 'branches' AND organizationId = ?";
    return await executeQuery(sqlQuery,[organizationId]);
}



export async function insertBranchesTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const { status, results } = validateDataArray([templateContent], templateContentsInsertSchema, messagesEn);
    const newTemplateContent = { ...templateContent, scopeName: 'branches' };
    if (status) {
...
export async function updateBranchesTemplateContent( templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const { status, results } = validateDataArray([templateContent], templateContentsUpdateAndDeleteSchema, messagesEn);
    if (status) {
        const columKey = { id: templateContent.id,organizationId: templateContent.organizationId, scopeName: 'branches'};
...
export async function deleteBranchesTemplateContent(templateContent: templateContent): Promise<{ data: Object | null, status: boolean, errorCode: string | Object }> {
    const columKey = { id: templateContent.id,organizationId: templateContent.organizationId, scopeName: 'branches'}; 
