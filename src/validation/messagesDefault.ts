...
export const messagesVi : TranslateMessageMap = {
  ...
  min: "{label} lớn hơn {min}.",
  max: "{label} nhỏ hơn {max}.",
  minLength: "{label} cần tối thiểu {minLength} ký tự.",
  maxLength: "{label} không được vượt quá {maxLength} ký tự.",
...
export const messagesEn :TranslateMessageMap = {
  ...
  maxLength: "{label} must not exceed {maxLength} characters long.",
